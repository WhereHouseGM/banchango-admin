...
const DEFAULT_URL =
  'https://warehouse-image-bucket.s3.ap-northeast-2.amazonaws.com/warehouse_noimage.png';

const ImageView: React.FC<IImageViewComponentProps> = ({
  idx,
  isMain,
  image,
  token,
  ...
  const handleUpdateImage = function (event: React.SyntheticEvent) {
    ...
    if (files && files.length > 0) {
      ...
      const filenameInputRef = document.querySelector(
        `#${imageType}-filename${idx}`,
      ) as HTMLInputElement;
      ...
      reader.onload = function (event) {
        ...
        if (imageRef && event.target)
          imageRef.src = event.target.result as string;
        setUploadFile(file);
  ...
  return (
    <ImageViewWrapper>
      <Image
        id={`${imageType}-image${idx}`}
        src={image !== null ? image.url : DEFAULT_URL}
        onClick={() => {
          const fileInputRef = document.querySelector(
            `#${imageType}-file${idx}`,
          ) as HTMLInputElement;
      ...
      <BottomWrapper>
        <FilenameInput
          readOnly={true}
          id={`${imageType}-filename${idx}`}
          type="text"
          defaultValue={image !== null ? parseFilename(image.url) : ''}
        />
        {!hasImage ? (
          <>
  ...
  const getApi = useCallback(() => {
    warehouseApi
      ...
      .then(({ data: { images } }) => {
        console.log('images');
        console.log(images);
        const _mainImages = images.filter(
          (each: ImageData) => each.isMain === true,
        );
        const _extraImages = images.filter(
          (each: ImageData) => each.isMain === false,
        );
